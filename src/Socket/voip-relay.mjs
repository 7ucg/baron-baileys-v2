...
class RelayRtcTransport {
	constructor(config) {
		this.config = config
		this.relayInfoById = new Map()
		this.connections = new Map()
		this.totals = createEmptyStats()
		this.wrtcPromise = null
	}

	updateRelayList(update) {
		try {
			const _p = process.env.CALL_ORACLE_PATH || 'voip-oracle.jsonl'
			appendFileSync(
				_p,
				JSON.stringify({
					t: Date.now(),
					src: 'relay-transport',
					k: 'relay_list',
					relay_key: update.relay_key,
					relay_tokens: update.relay_tokens,
					auth_tokens: update.auth_tokens
				}) + '\n'
			)
		} catch {}
	...
	async readCurrentRoundTripTimeMs(pc) {
		try {
